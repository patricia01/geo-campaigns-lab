 ...
 * =============================================================
 */
object GeoCampaignPipelineSolution extends App {
  ...
  val consentsDF = spark.read.parquet("/tmp/geo-campaigns/consents")
  val cellsDF    = spark.read.parquet("/tmp/geo-campaigns/cells")
  ...
  val rawStream = spark.readStream
    ...
    .option("maxOffsetsPerTrigger", 1000)   // ← máximo 50 mensajes por batch
  ...
  candidates.toDF().writeStream
    .format("console")
    .outputMode("append")
    .trigger(Trigger.ProcessingTime(3, TimeUnit.SECONDS))  // ← cada 10s
    .foreachBatch { (batchDF: DataFrame, batchId: Long) =>
      println(s"[Batch $batchId] 📥 Candidatos recibidos: ${batchDF.count()}")
...
*/
  ...
  // ──────────────────────────────────────────────────────────
  val cleanedStream = geoEvents.filter {geo =>
    geo.clientId != None &&
      !geo.clientId.isEmpty &&
      geo.signalDbm != None &&
      geo.congestion.getOrElse(-1) >= 0 &&
      geo.congestion.getOrElse(-1) <= 100
  ...
  val dedupedStream = cleanedStream
    ...
    .dropDuplicates("clientId","cellId","eventTime")  // TODO: campos para deduplicar
  ...
  // ──────────────────────────────────────────────────────────
  val aggregated = dedupedStream
    .groupBy(
      window(col("eventTime"), "2 minutes", "30 seconds"),
      col("clientId"),
      col("cellId")
    )
    .agg(
      avg("congestion")  .as("avgCongestion"),
      avg("signalDbm")   .as("avgSignalDbm"),
      last("technology") .as("technology"),
      count("*")         .as("eventCount")
    )
    .withColumn("avgCongestion", col("avgCongestion").cast(IntegerType))
    .withColumn("avgSignalDbm",  col("avgSignalDbm").cast(IntegerType))
  ...
  val withConsents = aggregated
    .join(broadcast(consentsDF), Seq("clientId"), "inner")
    .filter(col("hasConsent") === true)

  val withCells = withConsents
    .join(broadcast(cellsDF), Seq("cellId"), "inner")
    .filter(col("campaignActive") === true)
