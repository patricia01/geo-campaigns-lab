 ...
 */
case class GeoEvent(
                     clientId   : Option[String],   // ID del cliente (puede llegar nulo si dato sucio)
                     cellId     : String,   // ID de la antena
                     latitude   : Double,   // Latitud aproximada de la celda
                     longitude  : Double,   // Longitud aproximada
                     signalDbm  : Option[Int],      // Potencia de señal en dBm (puede llegar nulo)
                     congestion : Option[Int],      // % de saturación de la celda (0-100)
